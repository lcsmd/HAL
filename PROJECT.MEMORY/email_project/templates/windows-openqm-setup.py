...
class OpenQMSetupWindows:
    def __init__(self, qm_account='EMAILSYS'):
        self.qm_account = qm_account
        self.base_dir = Path('C:/email_management_system')
        ...
        for filename, (modulo, separation) in self.qm_files.items():
            try:
                ...
                result = subprocess.run(
                    f'qm -A{self.qm_account} -KCREATE.FILE {filename} {modulo} {separation}',
                    capture_output=True,
                    text=True,
                    timeout=10,
...
"""
        ...
        try:
            # Use shell=True on Windows
            result = subprocess.run(
                f'qm -A{self.qm_account} -K{script_file}',
                capture_output=True,
                text=True,
                timeout=10,
                shell=True
    ...
    def create_python_venv(self):
        ...
        try:
            ...
            requirements = [
                ...
                'lxml>=4.9.0',
                'python-magic-bin>=0.4.14',  # Windows version
                'anthropic>=0.18.0',
                'flask>=2.3.0',
                'flask-cors>=4.0.0',
                'schedule>=1.2.0'
    ...
    def create_config_template(self):
        ...
        config_content = f"""# Email Management System Configuration
...
attachment_dir = {self.base_dir}\\attachments
html_objects_dir = {self.base_dir}\\html_objects
bodies_dir = {self.base_dir}\\bodies
...
"""
    ...
    def run_setup(self):
        ...
        print("="*60)
        print("AI-Enhanced Email Management System - Windows Setup")
        print("="*60)
        print()
        ...
        print(f"   {self.base_dir}\\venv\\Scripts\\activate.bat")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Setup AI Email Management System with OpenQM (Windows)')
    parser.add_argument('--account', default='EMAILSYS', help='OpenQM account name')
    
    args = parser.parse_args()
    
    setup = OpenQMSetupWindows(qm_account=args.account)
    setup.run_setup()
